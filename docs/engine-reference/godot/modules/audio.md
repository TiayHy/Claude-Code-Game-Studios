...
4.4–4.6 版本音频 API 无重大破坏性变更。核心音频系统保持稳定。主要更新为工作流改进：
...
### SFX 对象池
```gdscript
# 预创建多个 AudioStreamPlayer 节点以支持并发音效
...
## 常见错误
- 运行时创建新的 AudioStreamPlayer 节点而非使用对象池
- 未使用音频总线来管理音量分类（Music、SFX、UI、Voice）
- 在 `_process()` 中处理音频时序而非使用信号（`finished`）